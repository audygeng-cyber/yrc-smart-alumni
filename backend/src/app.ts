  ...
  app.get('/', (_req, res) => {
    res.json({
      ...
      paths: {
        ...
        finance:
          'GET /api/admin/finance/overview | … | exports/*.csv (UTF-8 — เปิด Excel ได้; ชุดผู้ตรวจ) | period-closing … — ขอบเขต: บันทึก/ส่งออกให้ผู้ตรวจปิดงบนอกระบบ ไม่รับรองงบในแอป — เส้นทางเต็ม: overview | bank-accounts | payment-requests | payment-requests/:id | journals | journals/:id | fixed-assets | fiscal-years | reports/pl-summary | reports/donations | reports/trial-balance | reports/general-ledger | reports/income-statement | reports/balance-sheet | reports/tax-monthly | exports/donations.csv | exports/payment-requests.csv | exports/meeting-sessions.csv | exports/auditor-package.csv | period-closing | period-closing/:id | period-closing/:id/auditor-package.csv | meeting-agendas | meeting-agendas/:id/vote-summary | meeting-sessions/:id/minutes | meeting-sessions/:id/minutes.txt | meeting-documents | meeting-documents/:id/download.txt (query: legal_entity_code, from, to, scope, status, meeting_session_id, agenda_id, limit, auditor_handoff_status), POST …/payment-requests | payment-requests/:id/approve | journals | journals/:id/lines | journals/:id/post | journals/:id/void | fixed-assets | fixed-assets/run-depreciation | fiscal-years | fiscal-years/:id/close | tax/calculate | period-closing | period-closing/:id/mark-auditor-sent | period-closing/:id/mark-auditor-completed | meeting-sessions | meeting-sessions/:id/sign-attendance | meeting-sessions/:id/minutes | meeting-sessions/:id/minutes/publish | meeting-agendas | meeting-agendas/:id/votes | meeting-agendas/:id/close | meeting-documents, PATCH …, DELETE …/meeting-documents/:id',
        memberRequests:
